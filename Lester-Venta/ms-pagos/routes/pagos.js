module.exports = (pool) => {
  ...
  // POST crear nuevo pagos
  router.post('/', async (req, res) => {
    try {
      const connection = await pool.getConnection();
      const [result] = await connection.query(`INSERT INTO pagos SET ?`, req.body);
      connection.release();
  ...
  // PUT actualizar pagos
  router.put('/:id', async (req, res) => {
    try {
      const connection = await pool.getConnection();
      await connection.query(`UPDATE pagos SET ? WHERE id = ?`, [req.body, req.params.id]);
      connection.release();
