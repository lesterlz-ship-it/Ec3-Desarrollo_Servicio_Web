...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || 'alejo_reclamos',
...
app.post('/api/reclamos', async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [result] = await connection.execute('INSERT INTO reclamos (data, fecha_creacion) VALUES (?, NOW())', [data]);
    connection.release();
...
app.put('/api/reclamos/:id', async (req, res) => {
  try {
    ...
    if (!data) return res.status(400).json({ success: false, error: 'Data requerida' });
    const connection = await pool.getConnection();
    await connection.execute('UPDATE reclamos SET data = ?, fecha_actualizacion = NOW() WHERE id = ?', [data, req.params.id]);
    connection.release();
...
app.delete('/api/reclamos/:id', async (req, res) => {
  try {
    const connection = await pool.getConnection();
    await connection.execute('DELETE FROM reclamos WHERE id = ?', [req.params.id]);
    connection.release();
...
app.get('/health', (req, res) => res.json({ status: 'OK', service: 'ms-reclamos' }));

const PORT = process.env.PORT || 3029;
app.listen(PORT, () => console.log(`✓ ms-reclamos en puerto \${PORT}`));
